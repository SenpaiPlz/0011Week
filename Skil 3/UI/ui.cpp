...
void UI::computerTable(vector<Computer>& tmp)
{
    //Header for table
    cout << left << "| " << setw(3) << "ID" << "| "
                 << setw(18) << "Name" << "| " << setw(5)
...
void UI::linkTable(vector<ComputerScientist>& temp, vector<Computer>& tmp, vector<int>& rowid)
{
    //Header for table
...
void UI::SearchMenu()
{
    ...
    while(true)
    {
        ...
        case '3':
        {
            ...
            QString newstr = QString::fromUtf8(str.c_str());
            vector<Computer> tmp = c.SearchComputer(newstr);
            computerTable(tmp);
            cout << "------------------------------------------------------------";
            cout << "\nPress ENTER to go back to SEARCH MENU.";
